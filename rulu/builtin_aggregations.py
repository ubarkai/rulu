...
class MapReduceFunc(BaseExpr):
    ...
    def __init__(self, *params):
        self.params = list(map(normalize_expr, params))
        ...
        self.map_values = list(map(normalize_expr, map_values))
...
class AssertAggregate(Assert):
    ...
    def prepare_rule(self, rule):
        self.data.update({key : normalize_expr(value) for key, value in rule.groupby.items()})
    ...
    def _create_reduce_rule(self, map_rule):
        ...
        reduce_kwargs = {}
        for field_name, (value, subfields) in self.map_reduce_values.items():
